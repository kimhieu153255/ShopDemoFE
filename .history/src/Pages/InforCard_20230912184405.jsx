...
const InforCard = () => {
  ...
  const changeColorByImg = (imgSrc) => {
    const inforProduct = product?.inforProducts.find(
      (item) => item.imgSrc === imgSrc
    );
    if (inforProduct) {
      setColorChosen(inforProduct.color);
    }
  };
      ...
      <div className="flex py-5 bg-gray-100 rounded-sm">
        ...
        <div className="flex flex-col gap-3 items-center justify-center mx-10 ">
          ...
          <div className="flex gap-1">
            {product?.inforProducts.map((item, index) => (
              <div
                className={
                ...
                key={index}
                onmou={() => {
                  setImgShow(item?.imgSrc);
                  changeColorByImg(item?.imgSrc);
                }}
              >
                <img
                  src={item?.imgSrc}
                ...
                <div className="flex gap-2">
                  {sizeArr.map((size, index) => (
                    <div
                      key={index}
                      className={
                        `w-8 h-8 rounded-full border-2  text-center font-semibold text-lg hover:text-white hover:bg-green-500  ` +
                        (sizeChosen === size
