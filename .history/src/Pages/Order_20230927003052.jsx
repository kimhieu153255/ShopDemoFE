    ...
    <div className="w-full h-full">
      {orderDate.map((item, index) => (
        <div className="box bg-gray-100 border rounded-md p-5 mb-2" key={index}>
          <div className="topTitle flex justify-between mb-2 ml-2">
            <div className=" flex gap-2">
              <span className="font-semibold text-lg">{item.nameStore}</span>
              <span className="border border-green-500 rounded-md px-3 py-0.5 hover:bg-green-500 hover:border-green-600">
                View Shop
              </span>
            </div>
          ...
          {item?.products?.map((product, ind) => (
            <div
              className="item flex items-center justify-between mb-2"
              key={ind}
            >
