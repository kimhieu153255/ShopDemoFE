  ...
  return (
    <div className="w-full h-full">
      {orderDate.map((item, index) => (
        <div
          className="box bg-gray-100 border rounded-md p-5 pl-2 pr-2 mb-2"
          key={index}
        >
          <div className="topTitle flex justify-between pb-5  border-b-2">
          ...
          {item?.products?.map((product, ind) => (
            ...
            >
              ...
              <div>
                VND{" "}
                <span>{(product?.price || 1) * (product?.quantity || 0)}</span>
              </div>
            </div>
          ))}
          <div className="flex justify-end pt-10 pb-5 gap-5 text-lg">
