...
const LoginUser = () => {
  ...
  const { user, setUser, setToken, cleanUp } = useContext(AuthContext);

  useEffect(() => {
    user && navigate("/");
  }, [navigate, user]);

  const LoadStoreId = (userId) => {
    //get storeByUserId (userId in query)
    const { data, err, loading } = useSWR(
      `http://localhost:3001/store/api/getStoreByUserId?userId=${userId}`,
      fetcher
    );
    console.log(data);
