...
export const CartSlice = createSlice({
  ...
  reducers: {
      ...
      tempCartItems.map((item) => {
        item.items.map((el) => {
          if (el.check) {
            ...
            countTemp += el.price * el.quantity;
          } else {
            state.BuyList = state.BuyList.filter((item) => item.id !== el.id);
            if (countTemp !== 0) countTemp -= el.price * el.quantity;
          }
        });
      });
      state.total = countTemp;
    },
    ...
    buyProduct: (state, action) => {
      ...
      const { check, id, colorChosen, sizeChosen, num, price, name } =
        action.payload;
      if (state.BuyList.filter((item) => item.id === id).length > 0) return;
      if (!check) {
