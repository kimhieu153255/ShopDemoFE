...
const Card = ({ id, imgSrc, title, events, cost, num, rate }) => {
  ...
  return (
    <div
      className="inline-block w-44 bg-gray-200 shadow-md hover:border hover:border-green-500 hover:ring-green-500 hover:outline-none rounded-lg cursor-pointer hover:scale-105 hover:border-b-0 hover:rounded-b-none transition-all duration-300"
    ...
    >
      ...
      <div className="relative">
        <div
          className="hidden absolute w-full z-10 border border-green-500 rounded-b-lg ring-1 hover:ring-green-600 ring-green-500 transition-all duration-300 fixed bottom-0 left-0 bg-white"
          ref={displayRef}
        >
          <button
            className="w-full bg-green-500 text-white font-semibold py-1.5 rounded-lg rounded-t-none hover:bg-green-600 z-10"
