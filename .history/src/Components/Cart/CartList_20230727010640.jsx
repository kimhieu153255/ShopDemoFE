...
const CartList = () => {
  ...
  const ItemsRef = useRef(change2D(ListCart));
  useEffect(() => {
    // set true cho Item hàng mà vị trí checkOneStore = true
    console.log(ItemsRef.current);
    console.log(checkOneStore);
    let temp = [...ItemsRef.current];
    for (let i = 0; i < checkOneStore.length; i++) {
      for (let j = 0; j < checkOneStore[i].length; j++) {
        temp[i][j].check = checkOneStore[i];
      }
    }
    // setItem(temp);
    console.log("New ", Item);
    console.log("New ", checkOneStore);
  }, [Item, checkOneStore]);
  return (
