...
const CartItem = ({ img, color, size, price, name, check, id, quantity }) => {
  ...
  const { checkedProduct } = useContext(CheckContext);
  return (
    <div className="min-w-max flex justify-around items-center gap-3 w-full m-auto py-2 px-4 hover:bg-gray-50 border rounded-lg">
      <CountProvider quantity={quantity}>
            ...
            <div className="font-semibold">
              ...
              {color &&
                ...
                color.map((item) => (
                  <div
                    key={item}
                    className={`inline-block border-2 px-2 py-1 rounded-md font-normal hover:border-green-500 hover:text-green-500 mr-1 ${colorChosen}`}
        ...
        <div>{price}</div>
        <div>
          <ButtonCart quantity={quantity} id={id} />
        </div>
