...
const CartList = () => {
  ...
  const [item, setItem] = useState(change2D(ListCart));
  const ItemsRef = useRef(functionRef);
  const functionRef = () => {
    checkOneStore.forEach((item, index) => {
      if (item) {
        for (let i = 0; i < ItemsRef.current[index].length; i++) {
          ItemsRef.current[index][i] = true;
        }
      }
    });
  };
  useEffect(() => {
    // set true cho Item hàng mà vị trí checkOneStore = true
    console.log(ItemsRef.current);
    console.log(checkOneStore);
  }, [checkOneStore, ItemsRef]);
  return (
