...
const CartItem = ({ img, price, name, check, id, color, size, num }) => {
  ...
  const [colorChosen, setColorChosen] = useState(color || null);
  const [sizeColor, setSizeColor] = useState({});
  const [currentSize, setCurrentSize] = useState();
  const [currentColor, setCurrentColor] = useState();
  const [sizeChosen, setSizeChosen] = useState(size || null);
  ...
  return (
    <div className="min-w-max flex justify-around items-center gap-3 w-full m-auto py-2 px-4 hover:bg-gray-50 border rounded-lg">
                  ...
                  return (
                    <div
                      ...
                      onClick={async () => {
                        ...
                        updateCart(id, item, sizeChosen, count);
                      }}
                    >
                      {item}
                    </div>
                ...
                {sizeColor.sizes.map((item) => (
                  <div
                    ...
                    onClick={() => {
                      ...
                      updateCart(id, colorChosen, item, count);
                    }}
                  >
                    {item}
                  </div>
          ...
          <button
            ...
            onClick={() => {
              ...
              if (count > 1) {
                ...
                updateCart(id, colorChosen, sizeChosen, count - 1);
              }
            }}
          >
            <FaMinus />
      ...
      >
        <div
          id="messageBox"
          className="bg-white m-auto p-4 rounded-md relative"
        >
      ...
      {message && (
        <div className="fixed top-0 left-0 w-full h-full bg-opacity-70 bg-black items-center justify-center hidden">
          <div className="bg-white m-auto p-4 rounded-md relative">
            <p className="mb-4 font-semibold">{message}</p>
            <button className="absolute w-5 h-5 text-sm rounded-full bg-red-400 -top-2 -right-2 text-white flex items-center justify-center hover:bg-red-500 focus:outline-none focus:ring-2 focus:ring-red-400 focus:border-transparent">
              <FaX />
            </button>
          </div>
        </div>
