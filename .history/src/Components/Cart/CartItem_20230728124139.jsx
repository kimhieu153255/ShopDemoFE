...
// eslint-disable-next-line react/prop-types
const CartItem = ({ img, color, size, price, name, check, id, quantity }) => {
  ...
  const { checkedProduct, encreaseQuantity, deleteProduct, buyProduct } =
  ...
  return (
    <div className="min-w-max flex justify-around items-center gap-3 w-full m-auto py-2 px-4 hover:bg-gray-50 border rounded-lg">
      <CountProvider quantity={quantity}>
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={() => {
              checkedProduct(id);
              console.log(id, color, size, quantity, price);
              buyProduct(check, id, colorChosen, sizeChosen, quantity, price);
            }}
          />
          <div>
            <img
        ...
        <button
          className="border p-2 px-3 rounded-lg bg-green-500 hover:bg-green-600 hover:ring-green-300"
