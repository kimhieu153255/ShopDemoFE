...
const CartProvider = (props) => {
  ...
  const loadData = async () => {
    ...
    const res = await axios.get(
      `http://localhost:20474/cart/api/get/${user._id}`
    );
    if (res.data) {
  ...
  const addToCart = async (id, color, size, quantity) => {
    const res = await axios.post(`http://localhost:20474/user/api/addToCart`, {
      userId: user._id,
      id,
      color,
      size,
    ...
    if (res.data) setCartItems((cartList) => [...cartList, res.data]);
  };
