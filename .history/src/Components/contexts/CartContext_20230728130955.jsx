...
function CartProvider(props) {
  ...
  function buyAllProductInStore(nameStore) {
    const Products = cartItems.filter((e) => e.store === nameStore);
    const updateBuyList = Products.map((e) => {
      e.items.map((e) => {
        if (e.check)
          setBuyList([
            ...buyList,
            { id: e.id, color: e.color, size: e.size, quantity: e.quantity },
          ]);
      });
    }
  }
